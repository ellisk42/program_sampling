...
\begin{figure}	
  \centering
  \subfloat[Sketch]{{\BUseVerbatim[fontsize=\footnotesize,fontfamily=courier]{ExampleSketch}}}\quad
  \subfloat[Specification]{{\BUseVerbatim[fontsize=\footnotesize,fontfamily=courier]{ExampleSpecification}}}\quad
  \subfloat[Program space]{\begin{tikzpicture}[level distance=1.2cm, grow=down,
    ...
    edge from parent/.style={-latex, thin, draw}
      ...
      \node (P) {$P_1$}
      child {node (Q) {$x$} edge from parent node[left] {$c^1_1\hphantom{i}$ }}
      child {node (S) {nand} 
    	child {node (e2) {$P_2$}
        	child {node {$x$} edge from parent node[left] {$c^2_1\hphantom{i}$}}
    		child {node (pp) {nand} edge from parent node[left] {$c^2_2$}}
            child {node {0} edge from parent node[left] {$c^2_3\hphantom{i}$}}
        }
        child {node (e3) {$P_3$}}
        edge from parent node[left] {$\hphantom{i}c_2^1$}
        %edge from parent node[left] {$c^1_3\;$}
    }
    child {node (R) {$0$} edge from parent node[right] {$c^1_3$}};
...
\node (ss) [below] at (pp) {\vphantom{I}$\hdots\hdots$};
  ...
  \subfloat[Constraints from sketch]{\[ \begin{aligned}
      &c_1^1\vee c_2^1\vee c_3^1\\
      &\overline{c_1^1\wedge c_1^2}\wedge \overline{c_1^1\wedge c_1^3}\wedge \overline{c_1^2\wedge c_1^3}\\
%      c_2^1\vee c_2^1\vee c_3^1\\
      %      \overline{c_2^1\wedge c_2^2}\wedge \overline{c_2^1\wedge c_2^3}\wedge \overline{c_2^2\wedge c_2^3}
&\cdots      \cdots
    \end{aligned} \]}\quad
  \subfloat[Constraints from specification]{\[ \begin{aligned}
      &x_1\Leftrightarrow 0\wedge P_1^{(x_1)}\Leftrightarrow 1\\
      &x_2\Leftrightarrow 1\wedge P_1^{(x_2)}\Leftrightarrow 0\\
...
&\hspace{0.9cm}      (P_1^{(x_2)}\Leftrightarrow \overline{P_2^{(x_2)}\wedge P_3^{(x_2)}})\\
  ...
  \subfloat[Constraint solution]{\[ \begin{aligned}
    &  c_1^1 = 0, c_1^2 = 1, c_3^1 = 0\\
     & c_1^2 = 1, c_2^2 = 0, c_3^2 = 0\\
      &c_1^3 = 1, c_2^3 = 0, c_3^3 = 0\\
      &\\
      &\text{{\ttfamily Prog = nand(x, x)}}
  \end{aligned} \]}
